...
func (r *UPSDeviceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	device.Status.NUTName = device.Name
	if result.accepted {
	...
	setReadyCondition(
		...
		false,
		"TelemetryPending",
		"device contract is accepted but live NUT telemetry is not implemented yet",
	)
	setDegradedCondition(&device.Status.Conditions, device.Generation, !result.accepted, result.reason, result.message)
