...
var _ = Describe("PowerInventoryEdge Controller", func() {
	Context("When reconciling a resource", func() {
		const (
			resourceName = "test-power-inventory-edge"
		)
		...
		powerinventoryedge := &powerv1alpha1.PowerInventoryEdge{}

		BeforeEach(func() {
			By("creating the custom resource for the Kind PowerInventoryEdge")
			err := k8sClient.Get(ctx, typeNamespacedName, powerinventoryedge)
			if err != nil && errors.IsNotFound(err) {
				resource := &powerv1alpha1.PowerInventoryEdge{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance PowerInventoryEdge")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &PowerInventoryEdgeReconciler{
				Client: k8sClient,
