...
separate diagnostic run. Instead, that run found a real capability gap: this build cannot address
IPv6 through `ping` at all, by any invocation, so a diagnostic-only commit (no connectivity
assertion) inventoried the guest's actual toolset before writing a third fix attempt.

Fixed: replaced the ping-based check with `curl -6 -v --interface <iface> --connect-timeout 5
"http://[<addr>]:22/"`, asserting only that curl's verbose output contains `Connected to` --
