...
Status: design rationale and first milestone, 2026-09-13. See `docs/tasks.md`'s `VM-3` entry for
current status.
...
- Real signal delivery (the projected Secret volume, `internal/nodeagent.InspectSignal`'s
  fresh/stale/wrong-node/missing-fields cases) -- this milestone never writes a signal at all.
- Real `reboot(2)` and hypervisor-confirmed shutdown evidence -- the actual high-severity part of
  `VM-3`. `node-agent-operand.md`'s `OD-27` records that the operator has no independent channel to
  learn what the actuator did; proving this live means watching the guest from outside (the QEMU
  process exiting on its own via ACPI poweroff, not via `SafeStop`/`SafeTeardown` killing it) and
  capturing the actuator's own gate/syscall log lines from the guest's serial console
  (`test/hadron/adapter.go` already pipes it to `stdout`/`stderr`, per `smoke_test.go`'s own
  comment) before the process disappears, since PEG gives no separate signal for
  "guest asked to power off" versus "process died."
- Negative controls (missing/expired/wrong-node signal, revoked approval) that must leave the guest
  running -- follows once real signal delivery exists to make these actually distinct from each
  other.
- The full `NodePowerAgent` DaemonSet, RBAC, and the real per-node Secret projection -- this
  milestone deploys a bare Pod with the production environment/security context, not the full
  rendered manifest, to isolate "does the real image run for real" from "is the rendered manifest
  correct," which is already covered by existing envtest/Kind coverage against fakes.
