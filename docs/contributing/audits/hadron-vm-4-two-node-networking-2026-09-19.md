...
Status: in progress, 32 live runs across two workflows, 2026-09-18/19. See `docs/tasks.md`'s `VM-4`
...
from the `KUBE-NWPLCY-*` chain) actually contains the cross-node peer pod's IP -- `ipset list`
itself is not installed on this guest OS, so this could not yet be checked directly. Investigation
continues; not yet closed.

## Open

- Install `ipset` on the guest (available via `apt`, per direct confirmation) and inspect
  `KUBE-SRC-*`/`KUBE-DST-*` set membership directly, to confirm or rule out a kube-router cross-node
  ipset population gap as the final remaining cause.
