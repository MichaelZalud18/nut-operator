...
| [Shared VM framework](vm-test-framework.md) | Source-grounded duplication inventory, module contracts, extraction decisions, and incremental adoption sequence | [VM-12 foundations](../../tasks-completed.md#vm-test-coverage); [VM-13 expansion](../../tasks-completed.md#vm-test-coverage); [comprehensive review](../audits/vm-framework-comprehensive-review-2026-09-20.md); [VM-8 migration](../../tasks.md#vm-test-coverage) |
...
| OD-16 | A warning plus an explicit exemption marker, not a hard failure. A node with no modeled `carries` path raises `CommunicationPathUnmodeled` and increments `communication_path_unmodeled_nodes`; `communicationPathExempt` opts a node out. Unlike a `PowerPlanningOrphan`, it can still be planned, but missing coverage limits the communication-safety constraints derivable under v1's `PL-21`. The warning does not promise path survival. Communication-device actuation is not required to account for loss of its supplying UPS | inventory `compiler.go`, IN-6, IN-12, PL-21 |
...
| OD-38 | A PodDisruptionBudget is advisory once a flow is enforcing: a budget-refused eviction falls back to `Delete`, which still honors `terminationGracePeriodSeconds`, and the override is named in the audit details. A UPS outage is the disruption a budget defers, and it cannot be deferred past the battery. Only the `DisruptionBudget` status cause is overridden — API Priority and Fairness throttling stays fatal. Replaces the retired `PL-25`, which described the inverse failure | executor drain path, `internal/kubeactions` |
