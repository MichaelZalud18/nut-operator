...
connection-management choices rather than separate domain models.

The next storage step is a controller-owned connector that resolves the configured
`PowerManagementCluster.spec.storage` mode, obtains the DSN/credentials for CNPG or external
PostgreSQL, applies migrations through the audit store, and records events without putting
PostgreSQL on the critical shutdown decision path.
