...
- Node shutdown signals are structured, node-bound, plan-hash-bound, execution-bound, and TTL-bound.
  The actuator rejects stale signals and signals intended for another node.
...
- PostgreSQL outages degrade audit durability but do not silently change the power plan. The
  shutdown-time audit spool/last-ditch durability path is the planned mitigation for records that
  cannot be written before power loss.
...
| Operator to PostgreSQL | Reconciliation continues only where policy allows audit degradation. The condition and audit backend state show degraded durability. Shutdown-time local spool support remains planned. |
...
- Shutdown-time audit spool with replay into PostgreSQL when connectivity returns.
