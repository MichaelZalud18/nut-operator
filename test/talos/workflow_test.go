...
// TestSmokeWorkflowReservesCleanupBudget checks the same safety invariants test/hadron's own
...
// third guest adapter makes the overlap self-evident.
func TestSmokeWorkflowReservesCleanupBudget(t *testing.T) {
	const file = "../../.github/workflows/talos-vm-boot-smoke.yml"
	const job = "boot"
	...
	jobSpec, ok := workflow.Jobs[job]
	if !ok {
		t.Fatalf("job %q not found in %s", job, file)
	}
	...
	if cleanupIndex < 0 || total >= jobSpec.Timeout {
		t.Fatalf("cleanup missing or no job-level margin: steps=%dm job=%dm", total, jobSpec.Timeout)
	}
}
