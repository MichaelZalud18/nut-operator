...
func TestHadronActuatorHaltsOnAcceptedSignal(t *testing.T) {
	...
	if err := waitForQMPShutdown(haltCtx, socketPath); err != nil {
		t.Fatalf("guest did not report a real SHUTDOWN within the budget -- the actuator armed but the guest did not actually halt (a container outside the host PID namespace can call reboot(2) successfully and leave the machine running): %v", err)
	}
	t.Log("confirmed: QEMU's own QMP socket reported a real guest-initiated SHUTDOWN, without this test stopping it")
