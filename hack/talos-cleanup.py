...
Identical to hack/hadron-cleanup.py except for the expected state-root prefix: kept as its own
small script rather than a shared, parameterized one so this safety check keeps checking an exact,
hardcoded prefix for each smoke-test family, not a caller-supplied one a typo could weaken.
"""
...
def cleanup(root):
    ...
    if not root.is_dir() or not root.name.startswith("talos-smoke-"):
        raise ValueError("expected a private talos-smoke-* state directory")
