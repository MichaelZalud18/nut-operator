...
spec = importlib.util.spec_from_file_location("cleanup", Path(__file__).with_name("talos-cleanup.py"))
...
class CleanupTest(unittest.TestCase):
    def test_exact_owner(self):
        root = Path("/tmp/talos-smoke-test")  # nosec B108 -- a Path value for owns_command's string matching, never read or written
    ...
    def test_stops_only_owned_process(self):
        with tempfile.TemporaryDirectory(prefix="talos-smoke-") as root:
